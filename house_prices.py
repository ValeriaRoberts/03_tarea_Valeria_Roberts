...
ordinal_col = ['GarageQual', 'GarageCond', 'BsmtQual', 'BsmtCond', 'ExterQual',
               'ExterCond', 'KitchenQual', 'FireplaceQu', 'PavedDrive',
               'Functional', 'Electrical', 'Heating', 'BsmtFinType1',
               'BsmtFinType2', 'Utilities']
...
OE = OrdinalEncoder(categories=[['No', 'Unf', 'LwQ', 'Rec', 'BLQ', 'ALQ',
                                 'GLQ']])
...
OE = OrdinalEncoder(categories=[['Slab', 'BrkTil', 'Stone', 'CBlock', 'Wood',
                                 'PConc']])
train_data['Foundation'] = OE.fit_transform(train_data[['Foundation']])
test_data['Foundation'] = OE.transform(test_data[['Foundation']])

OE = OrdinalEncoder(categories=[['MeadowV', 'IDOTRR', 'BrDale', 'Edwards',
...
try:
    ...
    logging.info("Train data is clean and saved")
except OSError:
    logging.error(f"Clean train data not saved")


try:
    test_data.to_feather('../data/clean/test_clean.feather')
    logging.info("Test data is clean and saved")
except OSError:
    logging.error(f"Test train data not saved")


train_data_shape = train_data.shape
print(f"Train data shape is (1460, 36): {train_data_shape == (1460, 36)}")
...
try:
    for node in candidate_max_leaf_nodes:
        ...
        print(score.mean())
        print(f"La efectividad del modelo es aceptable (>80%): {score.mean() > .8 }")
        logging.info('El modelo se ejecuto correctamente')
except:
    logging.critical('EL MODELO NO SE PUDO EJECUTAR REVISAR ASAP')
